#!/usr/bin/env python3
"""Compute character-level micro-F1 and generate project result figures/tables."""
...
def normalize(text: str) -> str:
    ...
    text = re.sub(rf"(?<=[{THAI_CHARS}])\s+(?=[{THAI_CHARS}])", "", text)
    return re.sub(r"\s+", "", text.lower())


def score(refs: list[str], preds: list[str], saved_cer: float) -> dict:
    refs_norm = [normalize(s) for s in refs]
...
# Scaling artifacts include the same baseline prediction and references per step.
steps = [300, 500, 750, 1000]
scaling = []
refs = None
base_preds = None
for step in steps:
    path = f"preset_a_layer2_step{step}{'_final' if step == 1000 else ''}_evaluation.json"
    doc = read_json(path)
    rows = doc["rows"]
...
base_summary = read_json("preset_a_layer2_step300_evaluation.json")["summary"]
base_bf16 = {
...
mem = read_json("step750_nf4_memory_latency.json")["variants"]
int8_mem = read_json("step750_llm_int8_memory_latency.json")["variants"]
benchmark_entries = {
...
summary = {
    "dataset": {"samples": 300, "source_offset": 250000},
    "f1_definition": f1_definition,
    "results": results,
}
(OUT / "project_f1_scores.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
with (OUT / "project_f1_results_table.csv").open("w", newline="", encoding="utf-8-sig") as f:
...
ax.set_title("Quantization quality–storage trade-offs (N = 300)")
...
for suffix in ("pdf", "png"):
    fig.savefig(OUT / f"figure_1_cer_vs_tensor_storage.{suffix}", bbox_inches="tight")
plt.close(fig)
...
axes[0].scatter([750], [cer_values[x.index(750)]], marker="*", s=150, color="#009E73", edgecolor="black", linewidth=0.4, zorder=4)
...
axes[1].scatter([750], [f1_values[x.index(750)]], marker="*", s=150, color="#009E73", edgecolor="black", linewidth=0.4, zorder=4)
axes[1].set_ylabel("Character-level micro-F1 (%) — higher is better")
axes[1].set_title("(b) Character-level micro-F1")
axes[1].legend(frameon=False)
for a in axes:
    a.set_xlabel("Engram checkpoint step")
    a.set_xticks(x)
    a.grid(True, color="#dddddd", linewidth=0.65, alpha=0.8)
fig.suptitle("Engram checkpoint scaling on the fixed Thai ASR evaluation set (N = 300)", fontsize=11)
for suffix in ("pdf", "png"):
    fig.savefig(OUT / f"figure_2_scaling_cer_f1.{suffix}", bbox_inches="tight")
plt.close(fig)
...
print("Saved: project_f1_scores.json, project_f1_results_table.csv, figure_1_cer_vs_tensor_storage.{pdf,png}, figure_2_scaling_cer_f1.{pdf,png}")
